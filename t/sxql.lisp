...
(ok (string= (yield (constraint :pk-person :primary-key :id))
	     "CONSTRAINT `pk-person` PRIMARY KEY (`id`)"))
(ok (string= (yield (constraint :pkPerson :primary-key (list :id2)))
	     "CONSTRAINT `pkperson` PRIMARY KEY (`id2`)"))
(ok (string= (yield (constraint :pkPerson :primary-key (list :id2 :id3)))
	     "CONSTRAINT `pkperson` PRIMARY KEY (`id2`, `id3`)"))

(let ((*quote-character* #\"))
  (ok (string= (yield (sxql:constraint :uc-email-user :unique :last-name))
	       "CONSTRAINT \"uc-email-user\" UNIQUE (\"last-name\")"))
  (ok (string= (yield (sxql:constraint :uc-email-user :unique '(:last-name)))
	       "CONSTRAINT \"uc-email-user\" UNIQUE (\"last-name\")"))
  (ok (string= (yield (sxql:constraint :uc-email-user :unique '(:id :last-name)))
	       "CONSTRAINT \"uc-email-user\" UNIQUE (\"id\", \"last-name\")")))

(setf sxql:*quote-character* nil)

(ok (string= (yield (constraint :chk-person :check '(:= :city "Sandnes")))
	     "CONSTRAINT chk-person CHECK (city = 'Sandnes')"))
(ok (string= (yield (constraint :chk-person :check '(:and (:>= :age 18) (:= :city "Sandnes"))))
	     "CONSTRAINT chk-person CHECK ((age >= 18) AND (city = 'Sandnes'))"))

(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id)))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id)"))
(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id) :on-delete :cascade))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id) ON DELETE CASCADE"))
(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id) :on-delete :restrict))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id) ON DELETE RESTRICT"))
(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id) :on-update :set-null))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id) ON UPDATE SET NULL"))
(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id) :on-update :no-action))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id) ON UPDATE NO ACTION"))
(ok (string= (yield (constraint :fk-person-order :foreign-key :person-id :references (list :persons :person-id) :on-update :no-action :on-delete :set-default))
	     "CONSTRAINT fk-person-order FOREIGN KEY (person-id) REFERENCES PERSONS(person-id) ON DELETE SET DEFAULT ON UPDATE NO ACTION"))
